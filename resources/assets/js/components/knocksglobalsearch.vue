...
<div class = "row" id = "sidebar_search_group_holder">
	<form :action = "asset('app/search')" method="get" id = "knocks_sidebar_search_form">
		<knockselinput
		class = "input-with-select knocks_tinny_top_padding knocks_side_padding"
		name = "q"
		placeholder="Search"
		has_slot
		inner_placeholder
		...
		@autocomplete = "searchTypingResults = $event; emit('short_search')"
		@input = "searchTypingMode = true"
		:scope ="['sidebar_search_box']"
		v-model="sidebarSearch" >
		<knocksvoicerecognition v-model = "sidebarSearchRecognition" :lang = "sideBarSearchLanguage" @recognition = "runVoiceSearch($event)"  slot = "labelside"></knocksvoicerecognition>
		...
		<transition enter-active-class = "animated zoomIn" leave-active-class = "animated zoomOut">
			<div class = "row knocks_xs_padding" v-if = "searchTypingResults.length != 0 && searchTypingMode">
				<ul class = "uk-list uk-list-divider" >
					<li v-for = "query in searchTypingResults" @click = "updateSearchQuery(query.c)" class = " white-text">
						<span :class = "[
					...
					<transition   enter-active-class = "animated slideInUp" leave-active-class = "animated slideOutLeft">
						<div>
							<div class = "row"  v-for = "(user,index) in sidebarSearchResult.users" :key = "index" v-if="inSidebarUserRange(index)" >
								<knocksuser as_label :user ="user"  ></knocksuser>
							</div>
  ...
  methods : {
  	emit(e){
  		let res = {
  			...
  			result : this.sidebarSearchResult , 
  			shortResult : this.searchTypingResults , 
  		}
  		this.$emit('input' , res)
  		if(e)
  			this.$emit(e , res)
  	...
  	searcher(){
  	  this.sidebarSearchControllers.update(arguments[0])
