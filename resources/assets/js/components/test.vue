<template>
  <div v-if = "userObject != null" >
    <div :class = "main_container" v-if = "!as_chip && !as_result && !as_report && !as_name && !as_callback" >
   <div v-if="show_image">
      <el-popover
      v-if = "!hide_popover"
      ref="userpopover"
...
props : {
  ...
  methods : {
    formatChatStatus(){
      if(this.userObject == null) return 'offline';
      if(this.userObject.status !== undefined && this.userObject.last_seen !== undefined && this.userObject.last_seen && !this.userObject.status){
        if(this.status == 'offline')
          return 'offline';
        let offset = new Date().getTimezoneOffset();
        let finalDate = moment(this.userObject).subtract(offset ,'m');
         this.chatStatus = finalDate < 3 ? 'online' : this.formatLastSeen(finalDate);
    ...
    formatLastSeen(d){
      if(d < 60) return d+' M';
      let offset = new Date().getTimezoneOffset();
      if(moment(this.userObject).subtract(offset ,'h') < 24) return  moment(this.userObject).subtract(offset ,'h') + 'H';
      if(moment(this.userObject).subtract(offset ,'d') < 30) return d+' D';
      if(moment(this.userObject).subtract(offset ,'M') < 12) return d+' M';
      return moment(this.userObject).subtract(offset ,'y') + ' Y';
    },
