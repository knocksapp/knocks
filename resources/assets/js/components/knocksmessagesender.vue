...
<transition   name="custom-classes-transition" enter-active-class="animated zoomInDown" leave-active-class="animated zoomOutUp">
      ...
      <div class = "">
        ...
        <div :class="btn_container_class">
          ...
          <span slot = "content"  class = "knocks_tooltip animated flipInX " style="margin-top: 20px !important; left : -30px !important;" >
            ...
            <static_message msg = "Send"></static_message>
          </span>
          </knockspopover>
  ...
  <div :id="gid+'_location_modal'" class="modal modal-fixed-footer">
    <div class="modal-content">
      ...
      knocksclass="knocks_teal_input_sender" icon = "knocks-search12" >
      </knocksinput>
...
export default {
  ...
  props : {
  	...
  	btn_error_class : {
  		type : String , 
  		default : 'knocks_input_sender_error animated shake'
  	},
  	btn_success_class : {
      type : String ,
      default : 'knocks_input_sender_success '
    } ,
    btn_label_classes : {
    	type : String , 
  ...
  methods : {
      ...
      this.tokens =  $('#'+this.gid+'_input_sender').html().split(/ |&nbsp;/);
  	},
    getThebodyContent(){
      this.bodyContent =  $('#'+this.gid+'_input_sender').html().split(/&nbsp;/).join('');
    ...
    pushToTagged(user , index){
      ...
      let containerElement = document.getElementById(this.gid+'_input_sender');
      ...
      console.log(counter);
      let currentLen = $('#'+this.gid+'_input_sender').html().split(/&nbsp;/).join('');
    ...
    checkForTags(){
      let currentTags = document.getElementById(this.gid+'_input_sender').getElementsByClassName('knocks_tagged_user');
      let i, j, k, flag;
      let toBeSliced = [];
   ...
   prepareSubmit(){
    ...
    watchMyDomKeeper(){
       ...
       let childs = document.getElementById(vm.gid+'_input_sender').children;
       let i;
    ...
    watchMyDom(){
      const vm = this;
      document.getElementById(this.gid+'_input_sender').onpaste = function(){
      vm.watchMyDomKeeper();
      vm.watchMyDomKeeper();
    }
    $(document).on('keyup' , '#'+this.gid+'_input_sender' , function(){
      vm.watchMyDomKeeper();
      vm.watchMyDomKeeper();
      //$('#knockknock_input_sender').focus;
    });

    $(document).on('change' , '#'+this.gid+'_input_sender' , function(){
      vm.watchMyDomKeeper();
      vm.watchMyDomKeeper();
      //$('#knockknock_input_sender').focus;
    });
     $(document).on('focus' , '#'+this.gid+'_input_sender' , function(){
      vm.focused = true;

      if(!vm.interest)
      vm.showInterest();
     });
       $(document).on('blur' , '#'+this.gid+'_input_sender' , function(){
        vm.focused = false;
        vm.watchMyDomKeeper();
        vm.watchMyDomKeeper();
    });
 ...
 resetKnock(){
      ...
      $('#'+this.gid+'_input_sender').empty()
      $('#'+this.gid+'_input_sender').blur()
      this.notifi();
    ...
    notifi() {
        ...
        this.$message({
          message: h('p', null, [
            ...
            updateTextContent(){
      this.textContent.text= $('#'+this.gid+'_input_sender').text();
